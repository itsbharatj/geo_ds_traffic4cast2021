...
def run_model_with_custom_split(
    ...
    **kwargs,
    ...
    loss_function : Optional[torch.nn.Module]
        Custom loss function to use (e.g., MSELossWiedemann)
    **kwargs
...
def train_pure_torch(
    ...
    model.eval()
    test_metrics = _test_pure_torch(test_loader, device, model, static_mask=static_mask)
...
def _test_pure_torch(loader, device, model, static_mask=None):
    ...
    # Compute metrics
    metrics = calculate_metrics(all_outputs, all_targets, static_mask=static_mask)
    return metrics


def load_static_mask(city, data_raw_path):
    """
    Load static mask for a given city from raw data path.
    
    Parameters
    ----------
    city : str
        City name
    data_raw_path : str or Path
        Path to raw data directory
        
    Returns
    -------
    np.ndarray
        Static mask with shape (495, 436, 8) or None if not found
    """
    from pathlib import Path
    import numpy as np
    ...
    try:
        ...
        static_file = Path(data_raw_path) / city / f"{city}_static.h5"
        if not static_file.exists():
            logging.warning(f"Static file not found: {static_file}")
            return None
        
        # Load static roads data
        static_roads = load_h5_file(str(static_file))
        
        # Create mask from static roads (where road pixels are 1, others are 0)
        static_mask_once = np.where(static_roads[0] > 0, 1, 0)
        
        # Expand to all 8 channels
        static_mask = np.repeat(static_mask_once[:, :, np.newaxis], 8, axis=2)
        
        logging.info(f"Loaded static mask for {city} with shape {static_mask.shape}")
        return static_mask
    
    except Exception as e:
        logging.error(f"Error loading static mask for {city}: {e}")
        return None
